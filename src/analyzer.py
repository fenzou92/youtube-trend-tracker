# src/analyzer.py

import json
import re
from collections import Counter
from datetime import datetime
import sys
import os

sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
import config
from database import get_all_videos, get_all_channels


# ─────────────────────────────────────────
# UTILS
# ─────────────────────────────────────────

def parse_duration(duration_str):
    """
    Convertit une durée ISO 8601 (PT8M30S) en secondes.
    Ex: PT8M30S → 510 secondes
    """
    if not duration_str:
        return 0
    match = re.match(r'PT(?:(\d+)H)?(?:(\d+)M)?(?:(\d+)S)?', duration_str)
    if not match:
        return 0
    hours   = int(match.group(1) or 0)
    minutes = int(match.group(2) or 0)
    seconds = int(match.group(3) or 0)
    return hours * 3600 + minutes * 60 + seconds


def detect_topic(title, tags_json="[]"):
    """
    Détecte le sujet d'une vidéo à partir de son titre et ses tags.
    Retourne le sujet le plus probable ou 'other'.
    """
    text = title.lower()
    try:
        tags = json.loads(tags_json)
        text += " " + " ".join(tags).lower()
    except:
        pass

    scores = {}
    for topic, keywords in config.TOPICS_KEYWORDS.items():
        score = sum(1 for kw in keywords if kw.lower() in text)
        if score > 0:
            scores[topic] = score

    return max(scores, key=scores.get) if scores else "other"


def get_day_of_week(published_at):
    """Retourne le jour de la semaine d'une date ISO."""
    try:
        dt = datetime.fromisoformat(published_at.replace("Z", "+00:00"))
        return dt.strftime("%A")  # Monday, Tuesday...
    except:
        return "Unknown"


# ─────────────────────────────────────────
# 1. ANALYSE PAR SUJET
# ─────────────────────────────────────────

def analyze_topics(videos):
    """
    Pour chaque sujet détecté, calcule :
    - Nombre de vidéos
    - Vues moyennes
    - Likes moyens
    - Meilleur vidéo du sujet
    """
    print("\n📊 Analyse par sujet...")

    topic_stats = {}

    for video in videos:
        topic = detect_topic(video["title"], video.get("tags", "[]"))
        video["detected_topic"] = topic  # enrichit la vidéo

        if topic not in topic_stats:
            topic_stats[topic] = {
                "topic": topic,
                "video_count": 0,
                "total_views": 0,
                "total_likes": 0,
                "best_video": None,
            }

        stats = topic_stats[topic]
        stats["video_count"] += 1
        stats["total_views"] += video.get("view_count", 0)
        stats["total_likes"] += video.get("like_count", 0)

        # Garde la meilleure vidéo du sujet
        if stats["best_video"] is None or video["view_count"] > stats["best_video"]["view_count"]:
            stats["best_video"] = {
                "title": video["title"],
                "view_count": video["view_count"],
                "channel": video.get("channel_name", ""),
            }

    # Calcule les moyennes
    results = []
    for topic, stats in topic_stats.items():
        count = stats["video_count"]
        results.append({
            **stats,
            "avg_views": round(stats["total_views"] / count) if count else 0,
            "avg_likes": round(stats["total_likes"] / count) if count else 0,
        })

    # Trie par vues moyennes
    results.sort(key=lambda x: x["avg_views"], reverse=True)

    print(f"  → {len(results)} sujets détectés")
    return results


# ─────────────────────────────────────────
# 2. ANALYSE PAR JOUR DE PUBLICATION
# ─────────────────────────────────────────

def analyze_best_days(videos):
    """
    Calcule les vues moyennes par jour de publication.
    Permet d'identifier le meilleur jour pour poster.
    """
    print("\n📅 Analyse des meilleurs jours de publication...")

    day_stats = {}

    for video in videos:
        day = get_day_of_week(video.get("published_at", ""))
        if day not in day_stats:
            day_stats[day] = {"day": day, "video_count": 0, "total_views": 0}

        day_stats[day]["video_count"] += 1
        day_stats[day]["total_views"] += video.get("view_count", 0)

    results = []
    for day, stats in day_stats.items():
        count = stats["video_count"]
        results.append({
            **stats,
            "avg_views": round(stats["total_views"] / count) if count else 0,
        })

    results.sort(key=lambda x: x["avg_views"], reverse=True)
    print(f"  → Meilleur jour : {results[0]['day']} ({results[0]['avg_views']:,} vues en moyenne)")
    return results


# ─────────────────────────────────────────
# 3. ANALYSE DE LA DURÉE IDÉALE
# ─────────────────────────────────────────

def analyze_duration(videos):
    """
    Regroupe les vidéos par tranches de durée
    et calcule les vues moyennes par tranche.
    """
    print("\n⏱️  Analyse de la durée idéale...")

    def get_duration_bucket(seconds):
        if seconds < 60:       return "< 1 min (Short)"
        elif seconds < 300:    return "1-5 min"
        elif seconds < 600:    return "5-10 min"
        elif seconds < 900:    return "10-15 min"
        elif seconds < 1800:   return "15-30 min"
        else:                  return "> 30 min"

    bucket_stats = {}

    for video in videos:
        duration_sec = parse_duration(video.get("duration", ""))
        bucket = get_duration_bucket(duration_sec)

        if bucket not in bucket_stats:
            bucket_stats[bucket] = {"bucket": bucket, "video_count": 0, "total_views": 0}

        bucket_stats[bucket]["video_count"] += 1
        bucket_stats[bucket]["total_views"] += video.get("view_count", 0)

    results = []
    for bucket, stats in bucket_stats.items():
        count = stats["video_count"]
        results.append({
            **stats,
            "avg_views": round(stats["total_views"] / count) if count else 0,
        })

    results.sort(key=lambda x: x["avg_views"], reverse=True)
    print(f"  → Durée idéale : {results[0]['bucket']} ({results[0]['avg_views']:,} vues en moyenne)")
    return results


# ─────────────────────────────────────────
# 4. ANALYSE DES MOTS-CLÉS DANS LES TITRES
# ─────────────────────────────────────────

def analyze_title_keywords(videos, top_n=20):
    """
    Extrait les mots les plus fréquents dans les titres
    des vidéos les plus performantes (top 50%).
    """
    print("\n🏷️  Analyse des mots-clés dans les titres...")

    # Filtre les stopwords basiques
    STOPWORDS = {
        "the", "a", "an", "in", "of", "to", "and", "is", "i", "my",
        "me", "for", "on", "at", "with", "this", "it", "be", "are",
        "was", "you", "your", "from", "that", "as", "or", "but",
        "so", "if", "no", "not", "we", "our", "he", "she", "they",
        "have", "has", "do", "did", "will", "can", "just", "how",
        "what", "when", "where", "why", "all", "by", "more", "up",
        "|", "-", "#"
    }

    # Trie par vues et prend le top 50%
    sorted_videos = sorted(videos, key=lambda x: x.get("view_count", 0), reverse=True)
    top_videos = sorted_videos[:max(1, len(sorted_videos) // 2)]

    word_counter = Counter()

    for video in top_videos:
        words = re.findall(r'\b[a-zA-Z]{3,}\b', video["title"].lower())
        filtered = [w for w in words if w not in STOPWORDS]
        word_counter.update(filtered)

    results = [{"word": word, "count": count} for word, count in word_counter.most_common(top_n)]
    print(f"  → Top mots : {', '.join([r['word'] for r in results[:5]])}")
    return results


# ─────────────────────────────────────────
# 5. PIPELINE COMPLET
# ─────────────────────────────────────────

def run_analysis():
    """Lance toutes les analyses et retourne les résultats."""
    videos   = get_all_videos()
    channels = get_all_channels()

    if not videos:
        print("❌ Aucune vidéo en base. Lance d'abord database.py")
        return None

    print(f"\n🎯 Analyse de {len(videos)} vidéos sur {len(channels)} chaînes...")

    results = {
        "topics":    analyze_topics(videos),
        "best_days": analyze_best_days(videos),
        "durations": analyze_duration(videos),
        "keywords":  analyze_title_keywords(videos),
        "total_videos":   len(videos),
        "total_channels": len(channels),
    }

    # Résumé dans le terminal
    print("\n" + "="*50)
    print("📈 RÉSUMÉ DES TENDANCES")
    print("="*50)

    print("\n🔥 Sujets les plus performants :")
    for t in results["topics"][:3]:
        print(f"  {t['topic']:15} → {t['avg_views']:>10,} vues moy. ({t['video_count']} vidéos)")

    print("\n📅 Meilleurs jours pour publier :")
    for d in results["best_days"][:3]:
        print(f"  {d['day']:12} → {d['avg_views']:>10,} vues moy.")

    print("\n⏱️  Durées les plus performantes :")
    for d in results["durations"][:3]:
        print(f"  {d['bucket']:20} → {d['avg_views']:>10,} vues moy.")

    print("\n🏷️  Mots-clés tendance :")
    print("  " + ", ".join([k["word"] for k in results["keywords"][:10]]))

    return results


if __name__ == "__main__":
    run_analysis()